...
- 已实现功能：
    ...
    2. 根据物品配置表，显示物品对应的名称、描述，将物品按照类别放入不同tab中；

    3. 根据物品拥有数量，实现使用数量的修改，并且支持手动输入；

    4. 每个tab相互独立，来回切换不会相互影响选中的位置（仅维护一个全局的使用数量，所以切换tab会影响使用数量——参考ROK）

- 待实现功能

    1. 物品使用（重点需要测试使用完某种物品引起list的变化，查看背包时获得新物品）

    2. 物品按照不同的使用方式，选择不同的展示UI

    3. 根据配置表加载物品品质底图和Icon
