...
- 因为游戏一直是自己的兴趣，所以工作中自己的自驱动力也比较高，会从一个玩家的角度去看一个游戏应该被实现成什么样子，比如实习时实现过一些策划没有提到的需求等等。平时也比较注重知识的积累和体系结构，希望自己以后的技能点不仅仅局限在罗辑上，而是能做游戏偏底层的一些框架设计。除了玩游戏以外，比较喜欢打篮球、听音乐和创作一些感情上的表达，比如我目前想做一款游戏，但技术方面可能并不能达到《艾迪芬奇的记忆》、《跳舞的线》等等，我就会在豆瓣上写一些文字、做一些视频，比如科比退役的时候做过一些视频，个人的表达欲以及希望得到别人情感上的共鸣也是我走向游戏开发的一个重要原因。
- 腾讯开源的精神，游戏开发精粹，腾讯游戏学院
腾讯对创新的鼓励这些。
- 重感情的人。
- 坦诚。
- 完美主义
...
- 来银行的原因：有亲戚介绍农行成长空间大，待遇和工作环境都比较好
- 中美贸易战、香港问题，只有坚持共产党领导才行，老百姓才有生存空间
- 印象最深的困难，怎么解决的
- 说说你对敏捷开发的实践
- 介绍下一个对自己最有价值的项目，以及在这个过程中的角色
- 说说你觉得最有意义的技术书籍
...
## 专业知识
- 白盒测试和黑盒测试
    - 白盒测试：又名为结构测试，主要目的是发现软件程序编码过程中的错误。
    - 黑盒测试：又称为功能测试，主要检测软件的每一个功能是否能够正常使用。
    - 区别：
        ...
        - 软件的黑盒测试意味着测试要在软件的接口处进行。这种方法是把测试对象看做一个**关闭的盒子**，测试人员完全不考虑程序内部的逻辑结构和内部特性，只依据程序的需求规格说明书，检查程序的功能是否符合它的功能说明。
- 负载均衡Load Balance：

    - 负载均衡的含义是指将负载（工作任务）进行平衡、**分摊到多个操作单元上**，例如FTP服务器、Web服务器、企业核心应用服务器和其它主要任务服务器等进行运行，从而协同完成工作任务。
    
    - 负载均衡**构建在原有网络结构之上**，它提供了一种透明且廉价有效的方法扩展服务器和网络设备的带宽，加强网络数据处理能力，增加吞吐量，提高网络的可用性和灵活性。

- 缓存服务器
    - 缓存指的是将需要频繁访问的网络内容存放在离用户较近、访问速度更快的系统中，以提高内容访问速度的一种技术。缓存服务器就是存放频繁访问内容的服务器。
    - Web缓存服务器的应用模式主要是正向代理和反向代理
    - 正向代理：正向代理(Proxy)模式是代理用户访问internet，即将远端的内容预先保存到缓存服务器，之后客户端将本来要直接发送到internet上连接请求发送给代理服务器处理。正向代理的目的是加速用户在使用浏览器访问Internet时的请求响应时间，并提高广域网线路的利用率。正向代理浏览器无需和该站点建立联系，只访问到Web缓存即可。
    - 反向代理：反向代理(Reverse Proxy)模式是针对Web服务器加速功能的，在该模式中，缓存服务器放置在web应用服务器的前面，当用户访问web应用服务器的时候，首先经过缓存服务器，并将用户的请求和应用服务器应答的内容写入缓存服务器中，从而为后续用户的访问提供更快的响应。即客户A访问代理服务器B，B再转发到目标服务器C上，并将C的应答内容存储在B中，整个过程其实A并不知道C的存在，并且B还能够为接下来相同的请求继续服务。
    
    ![img](assets/reverse-proxy.png)
    
- web服务器和应用服务器

    通俗的讲，Web服务器传送页面使浏览器可以浏览，然而应用程序服务器提供的是客户端应用程序可以调用(call)的方法(methods)。确切一点的说：Web服务器专门处理HTTP请求(request)，但是应用程序服务器是通过很多协议来为应用程序提供商业逻辑(business logic)。

- 分布式数据库
    - 分布式数据库系统 （DDBS）包含分布式数据库管理系统（DDBMS）和分布式数据库（DDB)。
    - 一个分布式数据库在逻辑上是一个统一的整体，在物理上则是分别存储在不同的物理节点上。它的分布性表现在数据库中的数据不是存储在同一计算机的存储设备上，但从用户的角度看，一个分布式数据库系统在逻辑上和集中式数据库系统一样，用户可以在任何一个场地执行全局应用，就好像那些数据是存储在同一台计算机上一样
    - 分布式数据库系统适合于单位分散的部门，允许各个部门将其常用的数据**存储在本地**，从而提高响应速度，降低通信费用。分布式数据库系统与集中式数据库系统相比具有可扩展性，通过**增加适当的数据冗余，提高系统的可靠性**。集中式数据库要求尽量减少冗余度，因为冗余数据浪费存储空间，而且容易造成各副本之间的不一致性。而为了保证数据的一致性，系统要付出一定的维护代价。而在分布式数据库中却希望增加冗余数据，在不同的场地存储同一数据的多个副本，其原因包括：
...
- 分布式文件系统
    ...
    分布式文件系统（Distributed File System）是指文件系统管理的物理存储资源不一定直接连接在本地节点上，而是通过计算机网络与节点相连。分布式文件系统的设计基于**客户机/服务器**模式。一个典型的网络可能包括多个供多用户访问的服务器。另外，对等特性允许一些系统扮演客户机和服务器的双重角色。例如，用户可以“发表”一个允许其他客户机访问的目录，则它也会像一个服务器一样提供给其他客户机服务。

- CDN内容分发

    ![img](assets/cdn.png)

    CDN的全称是Content Delivery Network，即内容分发网络。其基本思路是尽可能避开互联网上有可能影响数据传输速度和稳定性的瓶颈和环节，使内容传输得更快、更稳定。CDN通过在网络各处放置节点服务器所构成的在现有互联网基础之上的一层智能虚拟网络，实时地根据网络流量和各节点的连接、负载状况以及到用户的距离和响应时间等综合信息将用户的请求重新导向离用户最近的服务节点上。其目的是使用户可就近取得所需内容，解决Internet网络拥挤的状况，提高用户访问网站的响应速度。

- 解决高并发访问量的系统架构设计
    - 应用和静态分离

        将静态资源保存到专门的服务器中，静态资源主要包括图片、视频、js、css和一些资源文件等，这些文件因为没有状态所以分离比较简单，可以直接存放到相应的服务器并使用专门的域名去访问。
...
- 抽象类和接口的区别
    ...
    - 实现接口的一定要实现接口里定义的所有方法，而实现抽象类可以有选择地重写需要用到的方法，一般的应用里，最顶级的是接口，然后是抽象类实现接口， 最后才是具体类实现。
...
- equals 与 == 的区别
    - ==：如果作用于基本数据类型（float、int、char等）的变量，则直接比较其存储的“值”是否相等；如果作用于引用类型的变量，则比较的是所指向的对象的地址。
    
    - equals方法：首先注意equals方法不能作用于基本数据类型的变量。如果没有对equals方法进行重写，则比较的依然是引用类型的变量所指向的对象的地址；诸如String、Date等类对equals方法进行了重写的话，比较的是所指向的对象的内容，也就是它们指向对象的值。
...
- list 和set 区别
    ...
    - 其中list和set的主要区别包括：
        ...
        - 容器是否有序：list是一个有序的容器，而set方法是无序容器，无法保证每个元素的存储顺序，TreeSet通过Comparator或者 Comparable维护了一个排序顺序
...
- HashMap、ConcurrentHashMap、HashTable的区别
    ...
    - ConcurrentHashMap是HashTable的一种优化，通过把整个Hash表分为N个Segment，可以提供相同的线程安全，但是效率提升N倍，默认提升16倍。
    - Hashtable的synchronized是针对整张Hash表的，即每次锁住整张表让线程独占，而ConcurrentHashMap允许多个修改操作并发进行，其关键在于使用了锁分离技术
...
- 运行时异常与一般异常有何异同
    ...
    - checked异常也就是我们经常遇到的IO异常，以及SQL异常都是这种异常。对于这种异常，JAVA编译器强制要求我们必需对出现的这些异常进行catch。所以，面对这种异常不管我们是否愿意，只能自己去写一大堆catch块去处理可能的异常。这类异常一般是外部错误,例如试图从文件尾后读取数据等,这并不是程序本身的错误,而是在应用环境中出现的外部错误
    - runtime exception：编译通过，可以不处理，但运行不通过，出现RuntimeException，通常是程序员出错。虚拟机接管会终止线程或主程序。如错误的类型转换、数组越界访问和访问空指针等。如果不对运行时异常进行处理，那么出现运行时异常之后，要么是线程中止，要么是主程序终止。

- java中堆和栈有什么区别
    - Java的堆是一个运行时数据区，类的对象从中分配空间。这些对象通过new、newarray、anewarray和multianewarray等指令建立，它们不需要程序代码来显式的释放。**堆是由垃圾回收来负责的**，堆的优势是可以动态地分配内存大小，生存期也不必事先告诉编译器，因为它是在运行时动态分配内存的，Java的垃圾收集器会自动收走这些不再使用的数据。但缺点是由于要在运行时动态分配内存，**存取速度较慢**。
    - 栈的优势是，**存取速度比堆要快**，仅次于寄存器，栈数据**可以共享**。但缺点是，存在栈中的数据大小与生存期必须是确定的，缺乏灵活性。栈中主要存放一些基本类型的变量（int, short, long, byte, float, double, boolean, char）和对象句柄。
...
- varchar和char的使用场景
    * varchar除了是变长外，两者还有没有其他区别？
    存储方面：varchar节省了存储空间，但需要使用1或2个额外字节记录字符串长度（小于255只用一个）
    性能方面：由于varchar行是变长的，在update时时可能使行变得更长，需要额外的工作，容易产生碎片

    怎么取决这个字段该用char还是用varchar？
    使用场景不一样：
    varchar在下面这些场景使用是合适的：
    字符串列的最大长度比平均长度大很多；
    列的更新很少，所以碎片不是问题；
...
[网址]https://www.cnblogs.com/inception6-lxc/p/8686156.html
