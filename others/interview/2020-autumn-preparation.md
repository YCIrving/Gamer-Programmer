...
- 修饰普通函数，表明函数的作用范围，仅在定义该函数的文件内才能使用。在多人开发项目时，为了防止与他人命名空间里的函数重名，可以将函数定义为 static。
...
- 先说一点树的高度和深度：
    ...
    ![img](assets/depth-height-of-binary-tree.png)

    - 节点的高度：表示该节点到其所属叶节点之间经过最多的**边**的数量，叶节点的高度为0。
    - 节点的深度：表示根节点到该节点经过的**边**的数量，根节点的深度为0。
    - 树的高度：等于根节点的高度，或者最深叶节点的深度。
    - 树的直径（diameter）/宽度（width）：任意两个叶节点之间最大的**节点数**，上图中树的宽度为6
- 满二叉树：每层节点都排满的二叉树
...
- 僵尸进程
    ...
    一个进程使用fork创建子进程，如果子进程退出，而父进程并没有调用wait或waitpid来获取子进程的状态信息，那么子进程的进程描述符仍然保存在系统中。这种进程称之为僵尸进程。

- 守护进程

    运行在后台的一种特殊进程。它独立于控制终端并且周期性地执行某种任务或等待处理某些发生的事件。它不需要用户输入就能运行，而且可以对整个系统或某个用户程序提供某种服务。Linux系统的大多数服务器就是通过守护进程实现的。常见的守护进程包括系统日志进程syslogd、web服务器进程httpd、邮件服务器进程sendmail和数据库服务器进程mysqld等。

## 虚拟内存及其优缺点

- 电脑中所运行的程序均需经过内存执行，若执行的程序占用的内存很大，则会导致内存消耗殆尽。为解决该问题，WINDOWS运用了虚拟内存技术，即拿出一部分硬盘空间来充当内存使用，这部分空间即称为虚拟内存。


- 优点：
    - 可以弥补物理内存太小的不足
    - 一定程度的提高反应速度
    - 减少对物理内存的读取从而保护内存，延长内存的使用寿命；
...
- `cp`：Copy，对文件进行复制，使用形式cp source target
- `mv`：Move，对文件或文件夹进行移动，如果文件或文件夹存在于当前工作目录，还可以对文件或文件夹进行重命名。
- `cat`：Concatenate and print files，cat用于在标准输出（监控器或屏幕）上查看文件内容
- `vim`：该命令主要用于文本编辑，
- `tail`：print TAIL(from last)，默认在标准输出上显示给定文件的最后10行，可以使用`tail -n N`来指定显示最后N行的内容。
- `less`：print LESS，按页或按窗口打印文件内容，可以使用Ctrl+F向前翻页，Ctrl+B向后翻页
- `grep`：在给定的文件中搜寻指定的字符串。`grep -i`在搜寻时会忽略字符串的大小写，而`grep -r`则会在当前工作目录的文件中递归搜寻指定的字符串。
- `find`：在给定位置搜寻与条件匹配的文件。可以使用`find -name`为区分大小写的搜寻，`find -iname`不区分大小。
- `tar`：能创建、查看和提取tar压缩文件。`tar -cvf`用来创建压缩文件，`tar -tvf`用来查看压缩文件，`tar -xvf`用来提取压缩文件。
- `gzip`：创建和提取gzip压缩文件，`gzip -d`用来提取压缩文件。
- `su`：Switch User，切换不同的用户。
- `uname`：显示出关于系统的重要信息，如内核名称、主机名、内核版本、处理机类型等等，使用`uname -a`可以查看所有信息。
- `df`：Disk space Free，查看文件系统中磁盘的使用情况，包括硬盘已用和可用的存储空间以及其它存储设备的信息。使用`df -h`将结果以更友好的方式显示。
- `ps`：ProcesseS，显示系统的运行进程，常用`ps -aux`来查看系统所有的进程
- `time`：测算一个命令（即程序）的执行时间
- `watch`：间歇性（默认2s）的执行程序或命令，并将输出结果以全屏的方式显示。通过`watch -n N`来指定间隔N秒运行指令
- `echo`：输出字符串
- `kill`：向某个工作（%jobnumber）或者是某个PID（数字）传送一个信号，通常用来终止程序
- `top`：Top processes，默认按照CPU的占用情况，显示占用量较大的进程,使用`top -u`来查看某个用户的CPU使用排名情况。
- `chgrp`：改变文件所属用户组
- `chown`：改变文件的所有者，与chgrp命令的使用方法相同，只是修改的文件属性不同
- `chmod`：改变文件的权限
- `shutdown`：关闭计算机，`shutdown -r`用于重启计算机。
...
- order by:排序，asc升序，desc降序（默认升序，可缺省）
...
- group by：分组，按照查询结果中的某一列（或多列）进行分组，值相等的为一组。必须配合“聚合函数”（sum()、count()、avg()等）同时使用，使用时至少需要指定一个分组标志字段。

    - 默认方式：
        ```SQL
        // 按照学号分组，查询每个学号的总成绩
        select 学号,SUM(成绩) from 选课表 group by 学号
        // 如果不加group by，则整张表会被分为一组，计算的是所有学生的成绩总和
        ```
    - having关键字：
    
        having只能用于group by子句，作用于组内，having条件子句可以直接跟函数表达式

        ```SQL
        // 查询平均成绩大于001号课程平均成绩的学号，并按平均成绩降序排序
...
- 主键：能够唯一标识表中某一行的属性或属性组，该字段没有重复值，且不允许为空，可以看做一个特殊的索引。因为主键所在的列里的每一个记录都是唯一的，所以在同一个表里只能有一个主键。
- 索引：相当于对指定的列进行排序，就好比一本书的目录，通过它可以快速查到结果，而不需要进行全表扫描，可以大大加快数据库的查询速度。
- 创建表时，主键默认建立唯一索引。
- 在需要经常查询的字段上建立索引，但不能在同一个字段上建立两个索引。
- 区别：
...
关系型（SQL，Structured Query Language）数据库中的数据都存在特定结构的**表**中，而非关系型数据库的数据存储则更加灵活和可扩展，存储方式可以是json文件、哈希表等。

## varchar和char的比较
- 特点：varchar是变长的，char是定长的
- 存储：varchar节省了存储空间，但需要使用1或2个额外字节记录字符串长度（小于255时只用一个）
- 性能：由于包含varchar的行是变长的，因此在update时可能使行变得更长，可能需要额外的工作，且容易产生碎片
...
- 不适合
    ...
    - where条件里用不到的字段不创建索引。
    - 经常增删改的字段。
    - 数据重复且分布平均的字段，因为如果某个数据列包含许多重复的内容，为它建立索引不会有太大的实际效果。

## 内连接、外连接、交叉连接
- 连接：在连接条件中使用比较运算符（包括=、>、<、<>、>=、<=、!>和!<）进行表间的比较操作，并查询与连接条件相匹配的数据时用到的操作。
- 内连接：又称普通连接或者自然连接，它使用比较运算符根据每个表共有的列的值匹配两个表中的行，没有null值，可以视为全连接的相反情况。根据比较运算符不同，分为等值连接和不等连接两种。
- 外连接：分为左连接（LEFT JOIN）或左外连接（LEFT OUTER JOIN）、右连接（RIGHT JOIN）或右外连接（RIGHT OUTER JOIN）、全连接（FULL JOIN）或全外连接（FULL OUTER JOIN）。
- 左连接：返回左表中的所有行，如果左表中行在右表中没有匹配行，则结果中右表中的列返回空值。
- 右连接：与左连接恰好相反，返回右表中的所有行，如果右表中行在左表中没有匹配行，则结果中左表中的列返回空值。
- 全连接：返回左表和右表中的所有行。当某行在另一个表中没有匹配行时，另一个表中的列返回空值。
- 交叉连接：不带WHERE条件子句时，它将会返回被连接的两个表的笛卡尔积，返回结果的行数等于两个表行数的乘积。带WHERE时，返回或显示的是匹配的行数。  
...
## 暖场问题
- 自我介绍
- 项目（自己做的内容，采用的技术语言、实现功能、遇到问题及解决思路，500字）
- 来银行的原因：有亲戚介绍农行成长空间大，待遇和工作环境较好
- 中美贸易战、香港问题：只有坚持共产党的领导老百姓才有生存的空间
- 印象最深的困难，怎么解决的
- 说说你对敏捷开发的理解和实践
- 介绍一个对自己最有价值的项目以及在这个项目中扮演的角色
- 说说你觉得最有意义的技术书籍
- 你认为服务端开发工程师应该具备的能力：基础知识、抽象、逻辑推理


## 专业知识
- 白盒测试和黑盒测试
    - 白盒测试：又名为结构测试，主要负责发现软件程序编码过程中的错误。
    - 黑盒测试：又称为功能测试，主要负责检测软件的每一个功能是否能够正常使用。
    - 区别：
        - 软件的白盒测试是对软件的过程性细节做细致的检查。这种方法是把测试对象看做一个**打开的盒子**，它允许测试人员利用程序内部的逻辑结构及有关信息，设计或选择测试用例，对程序的所有逻辑路径进行测试，通过在不同点检查程序状态，确定实际状态是否与预期的状态一致。
        - 软件的黑盒测试意味着测试要在软件的**接口处**进行。这种方法是把测试对象看做一个**关闭的盒子**，测试人员完全不考虑程序内部的逻辑结构和内部特性，只依据程序的需求规格说明书，检查程序的功能是否符合它的功能说明。
  
- 负载均衡Load Balance：

    - 负载均衡的含义是指将负载（工作任务）进行平衡、**分摊到多个操作单元上**，例如FTP服务器、Web服务器、企业核心应用服务器和其它主要任务服务器等，从而协同完成工作任务。
    
    - 负载均衡是**构建在原有网络结构之上**的，它提供了一种透明且廉价有效的方法来扩展服务器和网络设备的带宽，加强网络数据的处理能力，增加吞吐量，提高网络的可用性和灵活性。

- 缓存服务器
    - 网络中的缓存指的是将需要频繁访问的网络内容存放在离用户较近、访问速度更快的系统中，以提高内容访问速度的一种技术。缓存服务器就是存放频繁访问内容的服务器。
    - Web缓存服务器的应用模式主要分为正向代理和反向代理
    - 正向代理：正向代理(Proxy)模式是代理用户访问Internet，即将远端的内容预先保存到缓存服务器。访问时客户端将本来要直接发送到Internet上连接请求直接发送给代理服务器。正向代理的目的是加速用户在使用浏览器访问Internet时的请求响应时间，并提高广域网线路的利用率。命中缓存时浏览器无需再和该站点建立联系。
    - 反向代理：反向代理(Reverse Proxy)模式是为了加速Web服务器一种模式。在该模式中，缓存服务器放置在web应用服务器的前面，当用户访问web应用服务器时，请求会首先经过缓存服务器，并将用户的请求和应用服务器应答的内容写入缓存服务器中，从而为后续用户的访问提供更快的响应。即客户A访问代理服务器B，B再转发到目标服务器C上，并将C的应答内容存储在B中，之后其他用户再发送相同请求就可以直接从缓存服务器B上获得数据，而无需经过服务器C。整个过程A并不知道C的存在，并且B还能够为接下来相同的请求继续服务。
    
    ![img](assets/reverse-proxy.png)
    
- web服务器和应用服务器

    Web服务器传送页面使浏览器可以浏览，然而应用程序服务器提供的是客户端应用程序可以调用(call)的方法(methods)。确切的说：Web服务器专门处理HTTP请求(request)，但是应用程序服务器是通过很多协议来为应用程序客户端提供商业逻辑(business logic)。

- 分布式数据库
    - 分布式数据库系统（DDBS）包含分布式数据库管理系统（DDBMS）和分布式数据库（DDB)。
    - 一个分布式数据库在逻辑上是一个统一的整体，在物理上则分别存储在不同的物理节点上。从用户的角度看，一个分布式数据库系统在逻辑上和集中式数据库系统一样，用户可以在任何一个场地执行全局应用，就好像数据是存储在同一台计算机上一样。它的分布性表现在数据库中的数据实际上并不是存储在同一计算机的存储设备上的。
    - 分布式数据库系统适合于单位分散的部门，允许各个部门将其常用的数据**存储在本地**，从而提高响应速度，降低通信费用。分布式数据库系统与集中式数据库系统相比具有可扩展性，还可以通过**增加适当的数据冗余，即在不同的场地存储同一数据的多个副本。（集中式数据库则要求尽量减少冗余度，因为数据冗余会浪费存储空间，而且容易造成各副本之间的不一致性。而为了保证数据的一致性，系统要付出一定的维护代价。）其原因包括：
        - 提高系统的可靠性、可用性。当某一场地出现故障时，系统可以对另一场地上的相同副本进行操作，不会因一处故障而造成整个系统的瘫痪。
        - 提高系统性能。系统可以根据距离选择离用户最近的数据副本进行操作，减少通信代价，改善整个系统的性能。
        
- 分布式文件系统

    ![img](assets/distributed-file-system.png)

    分布式文件系统（Distributed File System）是指文件系统管理的物理存储资源不一定直接连接在本地节点上，还可以连接在与本地节点通过计算机网络相连的其他节点上。分布式文件系统的设计基于**客户端/服务器**模式，一个典型的网络可能包括多个供多用户访问的服务器。另外，对等特性允许一些系统扮演客户机和服务器的双重角色，例如，用户可以“发表”一个允许其他客户机访问的目录，那么它就会像一个服务器一样提供给其他客户机服务。

- CDN内容分发

    ![img](assets/cdn.png)

    内容分发网络（CDN，Content Delivery Network），其基本思路是尽可能避开互联网上有可能影响数据传输速度和稳定性的瓶颈和环节，使内容传输得更快、更稳定。首先网络各处会放置节点服务器，构成在现有互联网基础之上的一层智能虚拟网络。之后CDN会实时地根据网络流量和各节点的连接、负载状况以及到用户的距离和响应时间等综合信息将用户的请求重新导向到离用户最近的服务节点上。其目的是使用户可就近取得所需内容，解决Internet网络拥挤的状况，提高用户访问网站的响应速度。

- 解决高并发访问量的系统架构设计
    - 将应用和静态资源分离

        将静态资源（主要包括图片、视频、js、css和一些资源文件）保存到专门的服务器中，因为这些文件没有状态所以分离比较简单，可以直接存放到相应的服务器并使用专门的域名去访问。

        ![img](assets/high-concurrency-solution.png)
...
## java
- 抽象类和接口的区别
    - 接口是公开的，里面不能包含私有的方法或变量，是用于让别人使用的，而抽象类可以包含私有方法或私有变量。 
    - 实现接口一定要实现接口里定义的所有方法，而实现抽象类可以有选择地重写需要用到的方法，一般的应用里，最顶级的是接口，然后是抽象类来实现接口，最后才是具体类的实现。
    - 接口可以实现多重继承，而一个类只能继承一个超类。但类可以通过实现多个接口来达到多重继承的目的。
...
- equals 与 == 的区别
    - ==：如果作用于基本数据类型（float、int、char等）的变量，则直接比较其存储的“值”是否相等；如果作用于引用类型的变量，则比较它们所指向的对象的地址。
    
    - equals方法：首先注意equals方法不能作用于基本数据类型的变量。如果没有对equals方法进行重写，则比较的依然是引用类型变量所指向对象的地址；如果是诸如String、Date等对equals方法进行了重写的类，比较的就是所指向对象的内容，也就是它们指向对象的值。
    
- list 和set 区别

    - java中的集合共包含三大类，它们分别是Set（集），List（列表）以及Map（映射）。
        - `ArrayList = vector` in C++
        - `LinkedList = list` in C++
        - `TreeSet = set` in C++
        - `HashSet = unordered_set` in C++

    ![img](assets/list-and-set-in-java.jpg)
    ...
    - 其中list和set的主要区别包括：
        ...
        - 容器是否有序：list是一个有序的容器，而set方法是无序容器，无法保证每个元素的存储顺序，但TreeSet可以通过Comparator或者Comparable维护了一个排序的顺序

        ![img](assets/list-and-set-in-java2.jpg)


- HashMap、ConcurrentHashMap、HashTable的区别
    - HashMap和HashTable都是散列表，且key-value是无序的
    - HashMap的实现是不同步的，所以不是线程安全的。另外，其中的key和value都可以为null
    - HashTable的实现是同步的，因此是线程安全的。另外，其中的key和value都不可以为null
    - ConcurrentHashMap是HashTable的一种优化，通过把整个Hash表分为N个Segment，可以提供相同的线程安全特性，但是效率提升了N倍（默认提升16倍）。
    - Hashtable的synchronized是针对整张Hash表的，即每次锁住整张表让线程独占，而ConcurrentHashMap允许多个修改操作并发进行，其关键在于使用了锁分离技术，每次只锁一部分表。
...
- 创建线程的方式及实现
    ...
    3. 通过Callable和Future创建线程
        ...
        - 创建Callable实现类的实例，使用FutureTask类来包装Callable对象，该FutureTask对象封装了该Callable对象的call()方法的返回值。（FutureTask是一个包装器，它通过接受Callable来创建。同时它也实现了Future和Runnable接口。）
        - 使用FutureTask对象作为Thread对象的target创建并启动新线程。
        - 调用FutureTask对象的get()方法来获得子线程执行结束后的返回值

- 运行时异常与一般异常有何异同

    ![img](assets/error-and-exception-in-java.jpg)

    - Throwable是java错误处理的父类，它包含两个子类：Error和Exception。其中Error指无法预期的严重错误，它会导致JVM虚拟机无法继续执行，而且几乎无法恢复捕捉。而Exception是可以恢复捕捉的，是令java程序健壮的手段。
    - checked异常：包括我们经常遇到的IO异常以及SQL异常。对于这种异常，java编译器强制要求我们必须进行catch。所以，面对这种异常不管我们是否愿意，都只能自己写一大堆catch块去处理可能的异常。这类异常一般并不是程序本身的错误，而是在应用环境中出现的错误，例如试图从文件尾后读取数据等。
    - runtime异常：这类异常可以通过编译编译且不处理，但无法通过运行。RuntimeException通常是程序员自己出错导致的，如错误的类型转换、数组越界访问和访问空指针等。如果不对运行时异常进行处理，那么虚拟机会进行接管，结果要么是线程中止，要么是主程序终止。

- java中堆和栈有什么区别
    - java中的堆是一个运行时数据区，类的对象会从中分配空间。这些对象通过new、newarray、anewarray和multianewarray等指令创建，而且它们不需要程序代码来显式释放，因为java中**堆是由垃圾回收机制来负责的**。堆的优势是运行时可以动态地分配内存大小，生存期也不必事先告诉编译器，java的垃圾收集器会自动收走这些不再使用的数据。但缺点是由于要在运行时动态分配内存，堆的**存取速度较慢**。
    - 相比之下，栈的优势是，**存取速度比堆要快**，仅次于寄存器，且栈的数据**可以共享**。但缺点是存在栈中的数据大小与生存期必须是确定的，缺乏灵活性，因此栈中主要存放一些基本类型的变量（int, short, long, byte, float, double, boolean, char）和对象句柄等。
