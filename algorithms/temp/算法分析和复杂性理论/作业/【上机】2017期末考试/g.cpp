...
int main() {
  int m, n1, n2, second, v, source, target, flow;
  double dis_max;
  double dis;
  double x1[110], y1[110], x2[110], y2[110];
  while (cin>>n1>> n2>> second>>v)
  {
      dinic_init();
      source = n1+n2;
      target = source +1;
      dis_max = 1.0 * second * v;
      for(int i=0; i<n1; i++)
      {
          cin>>x1[i]>>y1[i];
      ...
      {
          cin>>x2[i]>>y2[i];
          add_edge(n1+i, target, 1, 0);
      }
      for(int i=0; i<n1; i++)
      {
          for(int j=0; j<n2; j++)
          {
              dis= sqrt(pow(x1[i]-x2[j], 2) + pow(y1[i] - y2[j], 2));
//              cout<<dis<<endl<<dis_max<<endl;
              if(dis< dis_max)
              {
                  add_edge(i, n1+j, 1, 0);
...
//      print_graph(target+1);
      flow = dinic(source, target);
      printf("%d\n", n1-flow);
  }
//  int pig[M+1], pre[M+1];
//  bool con[N+1];
